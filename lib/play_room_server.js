...
const joker = new Card(0, "Joker", 0);
...
function startDaifugo(room) {
    ...
    daifugoData[room] = {order: playersList, index: 0, field: []}
    callPlayerName(room)
}
...
function handleHandSubmitting(socket) {
    ...
    function judgeSubmittedCard(room, submittedCardsIndex) {
        ...
        if (daifugoData[room].field.length != 0) {//場にカードがあるなら
            var isIncludingJoker = daifugoData[room].field.indexOf(joker) != -1;
            console.log(isIncludingJoker);
        }
    ...
    function isSequence(submittedCards) {
        switch (submittedCards.length){
            case 1:
                return true;
            case 2://階段は3枚以上から構成されるため
                return false;
        }

        var mark = submittedCards[0].mark;
        for(var i = 1; i < submittedCards.length; i++){
            if (mark !== submittedCards[i].mark && submittedCards[i].numberStrength != 0){
        ...
        for (var i = 1; i < submittedCards.length; i++) {
            ...
            if (num2 != 0 && Math.abs(num - num2) != 1) {//ジョーカーではなく差が1でないなら
                return false;
            }
            num = num2;
        }
