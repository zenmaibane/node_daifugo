...
function joinRoom(socket, room) {
    ...
    socket.broadcast.to(room).emit("message", {
        text: nickNames[socket.id] + "が " + room + "に入りました．"
    });
    var usersInRoom = io.sockets.clients(room); //同じルームに他の誰かがいるかの判定
    if (usersInRoom.length > 1) {
        var usersInRoomSummary = "現在の" + room + "には: ";
...
function startDaifugo(room) {
    ...
    daifugoData[room] = {playersList: playersList, index: 0, field: [], passCount: 0, revolution: false, winners:[], gameEnd:false}
    callPlayerName(room)
}
...
function turnNextPlayer(room, socket) {
    ...
    for(var i = 0; i<daifugoData[room].playersList.length;i++){
        for(var j = 0; j < daifugoData[room].winners.length; j++){
            if (daifugoData[room].playersList[i].id === daifugoData[room].winners[j].id){
                daifugoData[room].playersList.splice(i,1);
            }
        }
    }
    if (daifugoData[room].playersList.length == 1){
        gameEnd();
        return;
    }else if (daifugoData[room].index == daifugoData[room].playersList.length) {
        daifugoData[room].index = 0;
    }
    callPlayerName(room);
    socket.emit("card", {text: showCards(socket.hand)});
    function gameEnd() {
        io.sockets.emit("message", {text: nickNames[daifugoData[room].playersList[0].id] + "の負けです．"})
        io.sockets.emit("message", {text: "ゲームが終了しました．"})
        daifugoData[room].gameEnd = true;
    }
}
...
function handleHandSubmitting(socket) {
    socket.on("handSubmit", function (info) {
        ...
        var data = daifugoData[room];
        if (data.gameEnd){
            socket.emit("message", {text: "ゲームは終了しています．"})
            return;
        }
        var playerId = data.playersList[data.index].id;
        if (playerId !== socket.id) {
            socket.emit("message", {text: "あなたの番ではありません．"})
            return;
        }
    ...
    function judgeSubmittedCard(room, submittedCardsIndex) {
        ...
        if (socket.hand.length == 0){
            daifugoData[room].winners.push(socket);
            io.sockets.emit("message",{text: nickNames[socket.id] + "がアガリました．"})
        }
